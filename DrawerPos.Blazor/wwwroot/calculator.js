    ...
    Swal.fire({
        html: `
            <div class="rounded-lg shadow-md p-4 w-full bg-white">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <h2 class="text-lg font-semibold">ยอดเงินที่ต้องชำระ ${total}</h2>
                    <span></span>
                </div>
                <input id="balance-display" type="text" class="bg-gray-100 text-center text-blue-500 text-xl font-bold py-4 border-gray-300 mb-6 w-full" readonly value="0 ฿"/>
                ...
                <button id="open-drawer-button" class="bg-blue-500 text-white py-2 mt-6 w-full rounded-lg flex items-center justify-center" data-total="${total}">
        ...
        didOpen: () => {
            document.getElementById('open-drawer-button').onclick = function () {
                const total = parseFloat(this.getAttribute('data-total').replace(/[^\d.-]/g, ''));
                callUpdateBalanceView(total);
            };
...
function callUpdateBalanceView(total) {
    ...
    if (balanceValue >= total) {
        Swal.fire({
            ...
            text: `ยอดเงินที่ต้องชำระ ${formatCurrency(total)}`,
        });
    } else {
        Swal.fire({
            icon: "error",
            title: "ยอดเงินไม่เพียงพอ",
            text: `ยอดเงินที่ต้องชำระ ${formatCurrency(total)} แต่คุณมี ${formatCurrency(balanceValue)}`
        });
    }
